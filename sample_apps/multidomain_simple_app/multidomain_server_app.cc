...
DEFINE_string(server_domain_name,
...
void print_ops() {
  ...
  printf("\t--client_domain_name=domain_name\n");
  printf("\t--server_domain_name=domain_name\n");
  printf(
...
#  endif
    ...
    if (!trust_mgr->initialize_new_domain(FLAGS_server_domain_name,
                                          purpose,
...
#  ifdef DEBUG3
    printf("\nget-certified\n");
#  endif
    if (!trust_mgr->initialize_existing_domain(FLAGS_server_domain_name)) {
    ...
    // now client domain
    string client_domain_name = FLAGS_client_domain_name;
    string client_cert_file_name(FLAGS_data_dir);
    client_cert_file_name.append(FLAGS_secondary_cert_file);
    string client_domain_cert;
    if (!read_file_into_string(client_cert_file_name, &client_domain_cert)) {
    ...
    int    client_port = FLAGS_secondary_policy_port;
    string client_host = FLAGS_secondary_policy_host;

    if (!trust_mgr->add_or_update_new_domain(FLAGS_client_domain_name,
                                             purpose,
                                             client_domain_cert,
                                             client_host,
                                             client_port)) {
...
#  endif
    ...
    printf("Running App as server\n");
    if (!server_dispatch(FLAGS_server_domain_name,
                         FLAGS_server_app_host,
                         FLAGS_server_app_port,
                         *trust_mgr,
                         server_application)) {
