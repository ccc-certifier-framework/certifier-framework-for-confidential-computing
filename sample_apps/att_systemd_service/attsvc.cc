...
#else
#define ATT_LOG(priority, format, ...)  do {                                           \
  openlog("VMware Attestation Service", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);  \
...
static bool certifier_notification(cc_trust_data* app_trust_data, bool disk_encrypted)
{
  ...
  if (!channel.init_client_ssl(app_config.client, app_config.client_port,
        app_trust_data->serialized_policy_cert_,
        app_trust_data->private_auth_key_,
        app_trust_data->private_auth_key_.certificate())) {
    ATT_LOG(LOG_INFO, "Failed to initialize SSL channel to notification agent.");
    return false;
  }
...
#define BUFFER_SIZE 1024
  ...
  while(fgets(buffer, sizeof(buffer), file) != NULL) {
    ...
    if (sscanf(conf_str, "certifier_port = %d", &app_config.certifier_port) == 1) {
      continue;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (!file_exists(ATTSERVICE_DATA_DIR)) {
    ATT_LOG(LOG_INFO, "Creating configuration directory: %s", ATTSERVICE_DATA_DIR);
  ...
  if (!app_trust_data->init_policy_key(initialized_cert, initialized_cert_size)) {
  ...
  if (file_exists(config_file)) {
    ...
    if (!parse_config(config_file)) {
      ATT_LOG(LOG_INFO, "Failed to parse configuration file. Use default config.");
    }
  }
  ATT_LOG(LOG_INFO, "Attestation Service Configuration:");
  ATT_LOG(LOG_INFO, "  Certifier host: %s:%d", app_config.certifier_host.c_str(),
