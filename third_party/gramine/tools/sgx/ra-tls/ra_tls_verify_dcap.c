...
void print_bytes(int n, uint8_t* buf) {
  for(int i = 0; i < n; i++)
    printf("%02x", buf[i]);
}

int gramine_verify_quote(size_t quote_size, uint8_t* quote, size_t* mr_size, uint8_t* mr) {
    int ret;
    ...
    bool allow_outdated_tcb  = 1;
    bool allow_debug_enclave = 1;
...
#ifdef DEBUG
    printf("%s: Quote Size: %d Quote:\n", __FUNCTION__, quote_size);
    print_bytes(quote_size, (uint8_t*)quote);
#endif
...
#ifdef DEBUG
    INFO("%s: Supplemental size: %d data:\n", __FUNCTION__, supplemental_data_size);
    print_bytes(supplemental_data_size, (uint8_t*)supplemental_data);
    printf("%s: Quote verification done with result %s\n", sgx_ql_qv_result_to_str(verification_result));
#endif
    ...
    if (ret < 0) {
        ERROR("%s: Quote verification failed with error %s\n", __FUNCTION__,
              sgx_ql_qv_result_to_str(verification_result));
        goto out;
    }
    ...
    *mr_size = sizeof(quote_body->report_body.mr_enclave);
    memcpy(mr, (void*)&quote_body->report_body.mr_enclave, sizeof(sgx_measurement_t));
    printf("Gramine verify copied SGX attributes size %d\n", *mr_size);
    ret = 0;

out:
    free(supplemental_data);
    printf("Verify done with successful result: %d\n", ret);
    return ret;
}
