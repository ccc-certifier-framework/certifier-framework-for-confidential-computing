...
DEFINE_string(config, "", "configuration registers");
...
bool get_key_from_cert_file(const string &in, key_message *k) {
  ...
  if (!read_file(in, &in_read, asn_cert)) {
    printf("%s() error, line %d, can't red %s\n",
           __func__,
           __LINE__,
           in.c_str());
    return false;
  ...
  if (!PublicKeyFromCert(cert, k)) {
    printf("%s() error, line %d, can't get key from cert\n",
           __func__,
           __LINE__);
    return false;
  }
...
int main(int an, char **av) {
  ...
  if (FLAGS_key_subject != "") {
    ...
    if (!get_platform_entity_from_file(FLAGS_platform_subject, &sub_ent)) {
      printf("%s() error, line %d, can't get platform entity\n",
             __func__,
             __LINE__);
      return 1;
    }
    ...
    if (!get_environment_entity_from_file(FLAGS_environment_subject,
                                          &sub_ent)) {
      printf("%s() error, line %d, can't get environment entity\n",
             __func__,
             __LINE__);
      return 1;
    }
