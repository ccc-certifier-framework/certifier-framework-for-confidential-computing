# Instructions for running scenario2

This document gives detailed instructions for running scenario1
in both test and full SEV environment.  It differs from the
files in ../scenario1 in the way the shell scripts are factored.
This factorization may be a bit better for adaptation to real
applications.  The basic certifier utility used is cf_utility.exe,
...
export EXAMPLE_DIR=$CERTIFIER_ROOT/vm_model_tools/examples/scenario2
```
...
run-test-scenario.sh.  It runs all the subordinate scripts described
in Sevprovisioning.
...
Almost all the variables are set within run-test-scenario2.sh.  To run it from scratch,
in the simulated sev environment type:

```shell
    ./run-test-scenario.sh  -tt simulated -bss 1 -ccf 1 -loud 1
```
...
```shell
    ./run-test-scenario.sh  -tt simulated -bss 0 -ccf 0 -loud 1
```
...
```shell
    ./run-test-scenario.sh  -tt simulated -bss 0 -ccf 0 -pk 0 -loud 1
```
...
needed flags. run-test-scenario.sh provides ALL the arguments needed by
...
```shell
    ./run-test-scenario.sh  -tt simulated -bss 0 -ccf 0
```
