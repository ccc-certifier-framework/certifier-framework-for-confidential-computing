...
bool cold_init_trust_info() {

  // Because policy_key.cc is included, the asn1 policy cert is in
  // from initialized_cert
...
bool cold_init() {
  if (!cold_init_trust_info()) {
    printf("cold_init_trust_info returned false\n");
    return false;
  }
...
bool certify_me() {
  ...
  // get platform cert:  This is the cert signed by the
  // platform provider (e.g.- Intel) that says the attestation
  // key is trusted.  How you get this signed statement is
  // platform dependent.
  ...
  claim_message c;
  platform_statement_str.assign(
      (char*)signed_platform_says_attest_key_is_trusted.serialized_claim_message().data(),
  ...
  request.set_service_address(FLAGS_server_address);

  // privateAppKey
  RSA* priRsaAppKey = RSA_new();
  if (!key_to_RSA(privateAppKey, priRsaAppKey)) {
    printf("certify_me error 13\n");
    return false;
  }
  int signed_pk_size = RSA_size(priRsaAppKey);
  byte signed_pk[signed_pk_size];
  memset(signed_pk, 0, signed_pk_size);
  if (!rsa_sha256_sign(priRsaAppKey, serialized_pk.size(), (byte*)serialized_pk.data(), &signed_pk_size, signed_pk)) {
...
int main(int an, char** av) {
  ...
  } else if (FLAGS_operation == "temp-test") {
    // remove this
    if (!warm_restart()) {
      printf("warm-restart failed\n");
      return 1;
