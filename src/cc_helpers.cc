...
#ifdef SEV_SNP
extern bool
              sev_Init(const string &platform_ark_der_file,
...
bool
certifier::framework::cc_trust_data::initialize_application_enclave_data(
    ...
    int           out_fd)
{
  ...
  if (!application_Init(parent_enclave_type, in_fd, out_fd)) {
    printf("initialize_application_enclave_data: Can't init "
           "application-enclave\n");
    return false;
  }
...
bool
certifier::framework::cc_trust_data::initialize_simulated_enclave_data(
    const string &attest_key_file_name,
    const string &measurement_file_name,
    const string &attest_endorsement_file_name)
{
  if (!cc_policy_info_initialized_) {
    printf("initialize_simulated_enclave_data: Policy key must be initialized "
           "first\n");
    return false;
  }
...
bool
certifier::framework::cc_trust_data::initialize_gramine_enclave_data(
    const int size,
    byte *    cert)
{
#ifdef GRAMINE_CERTIFIER
...
bool
certifier::framework::cc_trust_data::initialize_sev_enclave_data(
...
bool
certifier::framework::cc_trust_data::initialize_oe_enclave_data(
...
bool
certifier::framework::cc_trust_data::init_policy_key(int   asn1_cert_size,
                                                     byte *asn1_cert)
{
...
bool
certifier::framework::cc_trust_data::initialize_keystone_enclave_data(
    const string &attest_key_file_name,
    const string &measurement_file_name,
    const string &attest_endorsement_file_name)
{
#ifdef KEYSTONE_CERTIFIER
  if (!cc_policy_info_initialized_) {
    printf("initialize_keystone_enclave_data: Policy key must be initialized "
           "first\n");
    return false;
  }
...
bool
certifier::framework::cc_trust_data::initialize_islet_enclave_data(
    const string &attest_key_file_name,
    const string &measurement_file_name,
    const string &attest_endorsement_file_name)
{
...
void
certifier::framework::cc_trust_data::print_trust_data()
{
...
bool
certifier::framework::cc_trust_data::save_store()
{
...
bool
certifier::framework::cc_trust_data::fetch_store()
{
...
void
certifier::framework::cc_trust_data::clear_sensitive_data()
{
...
bool
certifier::framework::cc_trust_data::put_trust_data_in_store()
{
  ...
  if (purpose_ == "authentication") {
    ...
    if (!store_.update_or_insert(symmetric_key_tag, key_type, value)) {
      printf(
          "put_trust_data_in_store: can't set serialized app symmetric key\n");
      return false;
    }
...
bool
certifier::framework::cc_trust_data::get_trust_data_from_store()
{
  const string string_type("string");
  const string key_type("key");
  const string signed_claim_type("signed_claim");
...
bool
certifier::framework::cc_trust_data::cold_init(const string &public_key_alg,
                                               const string &symmetric_key_alg)
{
  ...
  if (purpose_ == "authentication") {
    ...
    symmetric_key_.set_key_type(symmetric_key_alg);
    symmetric_key_.set_key_format("vse-key");
    symmetric_key_.set_secret_key_bits(service_symmetric_key_,
                                       8 * num_key_bytes);
...
bool
certifier::framework::cc_trust_data::warm_restart()
{
  // fetch store
  if (!cc_policy_store_initialized_) {
    if (!fetch_store()) {
      printf("certifier::framework::cc_trust_data::warm_restart: Can't fetch "
             "store\n");
      return false;
    }
  }
  ...
  if (!get_trust_data_from_store()) {
    printf("certifier::framework::cc_trust_data::warm_restart: Can't get trust "
           "data from store\n");
    return false;
  }
  return true;
...
bool
certifier::framework::cc_trust_data::GetPlatformSaysAttestClaim(
    signed_claim_message *scm)
{
  ...
  if (enclave_type_ == "application-enclave") {
    ...
    if (!application_GetPlatformStatement(&size_out, out)) {
      printf("certifier::framework::cc_trust_data::GetPlatformSaysAttestClaim: "
             "Can't get PlatformStatement from parent\n");
      return false;
    }
    string sc_str;
...
bool
certifier::framework::cc_trust_data::recertify_me(const string &host_name,
                                                  int           port,
...
bool
certifier::framework::cc_trust_data::certify_me(const string &host_name,
                                                int           port)
{
  if (!cc_all_initialized()) {
    if (!warm_restart()) {
  ...
  if (enclave_type_ == "simulated-enclave"
    ...
    if (!GetPlatformSaysAttestClaim(
      ...
      printf(
          "certifier::framework::cc_trust_data::certify_me: Can't get signed "
          "attest claim\n");
      return false;
    }
    string str_s;
    if (!signed_platform_says_attest_key_is_trusted.SerializeToString(&str_s)) {
      printf("certifier::framework::cc_trust_data::certify_me: Can't serialize "
             "signed attest claim\n");
      return false;
    }
    evidence *ev = platform_evidence.add_assertion();
    if (ev == nullptr) {
      printf("certifier::framework::cc_trust_data::certify_me: Can't add to "
             "platform evidence\n");
      return false;
    }
    ...
    if (!gramine_platform_cert_initialized) {
      printf(
          "certifier::framework::cc_trust_data::certify_me: gramine certs not "
          "initialized\n");
      return false;
    }
    evidence *ev = platform_evidence.add_assertion();
    ...
    if (!plat_certs_initialized) {
      printf("certifier::framework::cc_trust_data::certify_me: sev certs not "
             "initialized\n");
      return false;
    }
    evidence *ev = platform_evidence.add_assertion();
    ...
    if (!cc_provider_provisioned_) {
      printf("certifier::framework::cc_trust_data::certify_me: Can't get "
             "pem-chain\n");
      return false;
    }
    if (pem_cert_chain != "") {
    ...
    printf("certifier::framework::cc_trust_data::certify_me: Unknown enclave "
           "type\n");
    return false;
  }
  ...
  if (purpose_ == "authentication") {
    ...
    if (!make_attestation_user_data(enclave_type_, public_auth_key_, &ud)) {
      printf("certifier::framework::cc_trust_data::certify_me: Can't make user "
             "data (1)\n");
      return false;
    }
#ifdef DEBUG
    ...
    if (!make_attestation_user_data(enclave_type_, public_service_key_, &ud)) {
      printf("certifier::framework::cc_trust_data::certify_me: Can't make user "
             "data (1)\n");
      return false;
    }
    ...
    printf(
        "certifier::framework::cc_trust_data::certify_me: neither attestation "
        "or authorization\n");
    return false;
  }
  ...
  if (!ud.SerializeToString(&serialized_ud)) {
    printf(
        "certifier::framework::cc_trust_data::certify_me: Can't serialize user "
        "data\n");
    return false;
  }
  ...
  int  size_out = 16000;
  byte out[size_out];
  if (!Attest(enclave_type_,
              serialized_ud.size(),
              (byte *)serialized_ud.data(),
              &size_out,
              out)) {
    printf(
        "certifier::framework::cc_trust_data::certify_me():%d: Attest failed\n",
        __LINE__);
    return false;
  }
  ...
  if (!construct_platform_evidence_package(enclave_type_,
                                           purpose_,
                                           platform_evidence,
                                           the_attestation_str,
                                           ep)) {
    printf("certifier::framework::cc_trust_data::certify_me: "
           "construct_platform_evidence_package failed\n");
    return false;
  }
  ...
  if (!open_client_socket(host_name, port, &sock)) {
    printf(
        "certifier::framework::cc_trust_data::certify_me: Can't open request "
        "socket\n");
    return false;
  }
  ...
  if (resp_size < 0) {
    printf("certifier::framework::cc_trust_data::certify_me: Can't read "
           "response\n");
    return false;
  }
  if (!response.ParseFromString(serialized_response)) {
    printf("certifier::framework::cc_trust_data::certify_me: Can't parse "
           "response\n");
    return false;
  }
  close(sock);
  ...
  if (response.status() != "succeeded") {
    printf("certifier::framework::cc_trust_data::certify_me: Certification "
           "failed\n");
    return false;
  }
  ...
  if (purpose_ == "authentication") {
    ...
    printf(
        "certifier::framework::cc_trust_data::certify_me: Unknown purpose\n");
    return false;
  }
...
bool
certifier::framework::cc_trust_data::init_peer_certification_data(
...
bool
certifier::framework::cc_trust_data::recover_peer_certification_data()
{
  return false;
}

bool
certifier::framework::cc_trust_data::get_peer_certification(
...
bool
certifier::framework::cc_trust_data::run_peer_certification_service(
    const string &host_name,
    int           port)
{
  return false;
...
bool
construct_platform_evidence_package(string &          attesting_enclave_type,
                                    const string &    purpose,
...
int
SSL_my_client_callback(SSL *s, int *al, void *arg)
{
  printf("callback\n");
  return 1;
}
...
bool
extract_id_from_cert(X509 *in, string *out)
{
  if (in == nullptr)
    return false;
...
bool
load_server_certs_and_key(X509 *       root_cert,
...
bool
certifier::framework::server_dispatch(
    const string &host_name,
    int           port,
...
bool
certifier::framework::secure_authenticated_channel::init_client_ssl(
    const string &host_name,
    int           port,
    string &      asn1_root_cert,
    key_message & private_key,
...
//    the key.
bool
certifier::framework::secure_authenticated_channel::load_client_certs_and_key()
{
  // Todo: Add other key types
  RSA *r = RSA_new();
...
void
certifier::framework::secure_authenticated_channel::
    server_channel_accept_and_auth(void (*func)(secure_authenticated_channel &))
{
...
bool
certifier::framework::secure_authenticated_channel::init_server_ssl(
    const string &host_name,
    int           port,
    string &      asn1_root_cert,
    key_message & private_key,
...
int
certifier::framework::secure_authenticated_channel::read(int size, byte *b)
{
  return SSL_read(ssl_, b, size);
}

int
certifier::framework::secure_authenticated_channel::read(string *out)
{
  return sized_ssl_read(ssl_, out);
}

int
certifier::framework::secure_authenticated_channel::write(int size, byte *b)
{
  return sized_ssl_write(ssl_, size, b);
}

void
certifier::framework::secure_authenticated_channel::close()
{
  ::close(sock_);
  if (ssl_ != nullptr) {
    SSL_free(ssl_);
...
bool
certifier::framework::secure_authenticated_channel::get_peer_id(string *out)
{
