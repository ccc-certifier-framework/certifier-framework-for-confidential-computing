...
#define CALLING_SEQUENCE                                                       \
  "GeneratePolicyKey.exe --algorithm=\"RSA\" "                                 \
...
int main(int an, char **av) {
  ...
  if (!WriteFileFromBlock(FLAGS_cloudproxy_key_file,
                          output.size(),
                          (byte *)output.data())) {
  ...
  // write it as protobuf too
  {
    ...
    privateMsg.mutable_public_key()->set_bit_modulus_size(
        FLAGS_modulus_size_in_bits);
    ...
    if (!WriteFileFromBlock(proto_file_name,
                            proto_out.size(),
                            (byte *)proto_out.data())) {
      printf("Can't write output file\n");
      ret_val = 1;
