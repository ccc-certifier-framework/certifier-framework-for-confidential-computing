...
void PrintOptions() {
  printf("Calling sequence: SelfSignPolicyCert.exe "
         ...
         "--key_file=input-file --policy_identifier=name "
         "--cert_file=output-file\n");
}
...
int main(int an, char **av) {
  ...
  req_message.mutable_key()->mutable_rsa_key()->set_exponent(
      (const char *)&expOut,
      sizeof(uint64_t));
  req_message.mutable_key()->mutable_rsa_key()->set_modulus(mod->data(),
                                                            mod->size());
  printf("\ncert request\n");
  ...
  if (!WriteFileFromBlock(FLAGS_cert_file,
                          output.size(),
                          (byte *)output.data())) {
