...
typedef uint32_t TPM_STARTUP_EFFECTS;
...
//
#define TPM_Vendor_Specific32 ((uint32_t)0x00000400)
#define TPM_Vendor_Specific8  ((byte_t)0x80)
...
#define TPM_RT_DELEGATE                                                        \
  ((TPM_RESOURCE_TYPE)0x00000007)  ///< The handle is for a delegate row. These
                                   ///< are the internal rows held in NV storage
                                   ///< by the TPM
...
#define TPM_PT_MIGRATE_RESTRICTED                                              \
  ((TPM_PAYLOAD_TYPE)0x06)  ///< The entity is a restricted-migration asymmetric
                            ///< key
...
#define TPM_ET_KEYHANDLE                                                       \
  ((uint16_t)0x0001)                     ///< The entity is a keyHandle or key
#define TPM_ET_OWNER ((uint16_t)0x0002)  ///< The entity is the TPM Owner
#define TPM_ET_DATA  ((uint16_t)0x0003)  ///< The entity is some data
#define TPM_ET_SRK   ((uint16_t)0x0004)  ///< The entity is the SRK
#define TPM_ET_KEY   ((uint16_t)0x0005)  ///< The entity is a key or keyHandle
#define TPM_ET_REVOKE                                                          \
  ((uint16_t)0x0006)  ///< The entity is the RevokeTrust value
#define TPM_ET_DEL_OWNER_BLOB                                                  \
  ((uint16_t)0x0007)  ///< The entity is a delegate owner blob
#define TPM_ET_DEL_ROW ((uint16_t)0x0008)  ///< The entity is a delegate row
#define TPM_ET_DEL_KEY_BLOB                                                    \
  ((uint16_t)0x0009)  ///< The entity is a delegate key blob
#define TPM_ET_COUNTER  ((uint16_t)0x000A)  ///< The entity is a counter
#define TPM_ET_NV       ((uint16_t)0x000B)  ///< The entity is a NV index
#define TPM_ET_OPERATOR ((uint16_t)0x000C)  ///< The entity is the operator
#define TPM_ET_RESERVED_HANDLE                                                 \
  ((uint16_t)0x0040)  ///< Reserved. This value avoids collisions with the
                      ///< handle MSB setting.
// TPM_ENTITY_TYPE MSB Values: The MSB is used to indicate the ADIP encryption
// sheme when applicable
#define TPM_ET_XOR ((uint16_t)0x0000)  ///< ADIP encryption scheme: XOR
#define TPM_ET_AES128                                                          \
  ((uint16_t)0x0006)  ///< ADIP encryption scheme: AES 128 bits
...
#define TPM_KH_TRANSPORT                                                       \
  ((TPM_KEY_HANDLE)0x40000003)  ///< The handle points to the EstablishTransport
...
#define TPM_KH_ADMIN                                                           \
  ((TPM_KEY_HANDLE)0x40000005)  ///< The handle points to the delegation
                                ///< administration auth
#define TPM_KH_EK                                                              \
  ((TPM_KEY_HANDLE)0x40000006)  ///< The handle points to the PUBEK, only usable
                                ///< with TPM_OwnerReadInternalPub
...
#define TPM_ST_DEACTIVATED                                                     \
  ((TPM_STARTUP_TYPE)0x0003)  ///< The TPM is to startup and set the deactivated
                              ///< flag to TRUE
...
#define TPM_PHYSICAL_PRESENCE_LIFETIME_LOCK                                    \
  ((TPM_PHYSICAL_PRESENCE)0x0080)  ///< Sets the physicalPresenceLifetimeLock to
                                   ///< TRUE
...
typedef struct tdTPM_DIGEST {
  byte_t digest[TPM_SHA1_160_HASH_LEN];
} TPM_DIGEST;
...
typedef byte_t tdTPM_AUTHDATA[20];
...
#define TPM_KEY_SIGNING ((uint16_t)0x0010)
...
#define TPM_KEY_LEGACY ((uint16_t)0x0015)
/// TPM_KEY_MIGRAGE SHALL indicate a key in use for TPM_MigrateKey
#define TPM_KEY_MIGRATE ((uint16_t)0x0016)
...
typedef struct tdTPM_PERMANENT_FLAGS {
  ...
  byte_t            deactivated;
  byte_t            readPubek;
  byte_t            disableOwnerClear;
  byte_t            allowMaintenance;
  byte_t            physicalPresenceLifetimeLock;
  byte_t            physicalPresenceHWEnable;
  byte_t            physicalPresenceCMDEnable;
  byte_t            CEKPUsed;
  byte_t            TPMpost;
  byte_t            TPMpostLock;
  byte_t            FIPS;
  byte_t            operator_;
  byte_t            enableRevokeEK;
  byte_t            nvLocked;
  byte_t            readSRKPub;
  byte_t            tpmEstablished;
  byte_t            maintenanceDone;
  byte_t            disableFullDALogicInfo;
} TPM_PERMANENT_FLAGS;
...
typedef struct tdTPM_STCLEAR_FLAGS {
  TPM_STRUCTURE_TAG tag;
  byte_t            deactivated;
  byte_t            disableForceClear;
  byte_t            physicalPresence;
  byte_t            physicalPresenceLock;
  byte_t            bGlobalLock;
} TPM_STCLEAR_FLAGS;
...
typedef struct tdTPM_STANY_FLAGS {
  TPM_STRUCTURE_TAG      tag;
  byte_t                 postInitialise;
  TPM_MODIFIER_INDICATOR localityModifier;
  byte_t                 transportExclusive;
  byte_t                 TOSPresent;
} TPM_STANY_FLAGS;
...
typedef struct tdTPM_STCLEAR_DATA {
  ...
  byte_t            disableResetLock;
  TPM_PCRVALUE      PCR[TPM_NUM_PCR];
  uint32_t          deferredPhysicalPresence;
} TPM_STCLEAR_DATA;
...
///
typedef byte_t TPM_LOCALITY_SELECTION;

#define TPM_LOC_FOUR  ((byte_t)0x10)
#define TPM_LOC_THREE ((byte_t)0x08)
#define TPM_LOC_TWO   ((byte_t)0x04)
#define TPM_LOC_ONE   ((byte_t)0x02)
#define TPM_LOC_ZERO  ((byte_t)0x01)
...
typedef struct tdTPM_MIGRATE_ASYMKEY {  // pos  len  total
  ...
  uint32_t         partPrivKeyLen;      //  41    4      45
  byte_t          *partPrivKey;         //  45 112-127 157-172
} TPM_MIGRATE_ASYMKEY;

///
/// Part 2, section 10.9: TPM_KEY_CONTROL
///
#define TPM_KEY_CONTROL_OWNER_EVICT ((uint32_t)0x00000001)
...
typedef struct tdTPM_IDENTITY_PROOF {
  ...
  byte_t        *labelArea;
  byte_t        *identityBinding;
  byte_t        *endorsementCredential;
  byte_t        *platformCredential;
  byte_t        *conformanceCredential;
} TPM_IDENTITY_PROOF;
...
//
#define TPM_TRANSPORT_ENCRYPT   ((uint32_t)BIT0)
#define TPM_TRANSPORT_LOG       ((uint32_t)BIT1)
#define TPM_TRANSPORT_EXCLUSIVE ((uint32_t)BIT2)
...
typedef struct tdTPM_CONTEXT_BLOB {
  ...
  byte_t           *additionalData;
  uint32_t          sensitiveSize;
  byte_t           *sensitiveData;
} TPM_CONTEXT_BLOB;
...
#define TPM_NV_INDEX_DIR          ((uint32_t)0x10000001)
#define TPM_NV_INDEX_EKCert       ((uint32_t)0x0000f000)
#define TPM_NV_INDEX_TPM_CC       ((uint32_t)0x0000f001)
#define TPM_NV_INDEX_PlatformCert ((uint32_t)0x0000f002)
#define TPM_NV_INDEX_Platform_CC  ((uint32_t)0x0000f003)
//
// Part 2, section 19.1.2: Reserved Index values
//
#define TPM_NV_INDEX_TSS_BASE        ((uint32_t)0x00011100)
#define TPM_NV_INDEX_PC_BASE         ((uint32_t)0x00011200)
#define TPM_NV_INDEX_SERVER_BASE     ((uint32_t)0x00011300)
#define TPM_NV_INDEX_MOBILE_BASE     ((uint32_t)0x00011400)
#define TPM_NV_INDEX_PERIPHERAL_BASE ((uint32_t)0x00011500)
#define TPM_NV_INDEX_GROUP_RESV_BASE ((uint32_t)0x00010000)
...
#define TPM_DEL_OWNER_BITS ((uint32_t)0x00000001)
#define TPM_DEL_KEY_BITS   ((uint32_t)0x00000002)
///
/// Part 2, section 20.2: Delegate Definitions
///
typedef struct tdTPM_DELEGATIONS {
  TPM_STRUCTURE_TAG tag;
  uint32_t          delegateType;
  uint32_t          per1;
  uint32_t          per2;
} TPM_DELEGATIONS;
...
typedef struct tdTPM_DELEGATE_OWNER_BLOB {
  ...
  TPM_DIGEST          integrityDigest;
  uint32_t            additionalSize;
  byte_t             *additionalArea;
  uint32_t            sensitiveSize;
  byte_t             *sensitiveArea;
...
#define TPM_FAMILY_ENABLE     ((uint32_t)0x00000002)
#define TPM_FAMILY_ADMIN      ((uint32_t)0x00000003)
#define TPM_FAMILY_INVALIDATE ((uint32_t)0x00000004)
...
typedef struct tdTPM_CAP_VERSION_INFO {
  ...
  uint16_t          specLevel;
  byte_t            errataRev;
  byte_t            tpmVendorID[4];
  uint16_t          vendorSpecificSize;
  byte_t           *vendorSpecific;
...
#define TPM_DA_ACTION_FAILURE_MODE (((uint32_t)1) << 3)
#define TPM_DA_ACTION_DEACTIVATE   (((uint32_t)1) << 2)
#define TPM_DA_ACTION_DISABLE      (((uint32_t)1) << 1)
#define TPM_DA_ACTION_TIMEOUT      (((uint32_t)1) << 0)
...
//
#define TPM_DA_STATE_INACTIVE ((byte_t)0x00)
#define TPM_DA_STATE_ACTIVE   ((byte_t)0x01)
...
typedef struct tdTPM_DAA_JOINDATA {
  byte_t     DAA_join_u0[128];
  byte_t     DAA_join_u1[138];
