...
void print_response(const char *name,
                    uint16_t    cap,
                    uint32_t    size,
                    uint32_t    code,
                    byte_t     *buf) {
...
#define IF_LESS_THAN_RETURN_FALSE(x, y)                                        \
  if ((int)x < (int)y)                                                         \
    return false;
#define IF_LESS_THAN_RETURN_MINUS1(x, y)                                       \
  if ((int)x < (int)y)                                                         \
    return false;
#define IF_NEG_RETURN_FALSE(x)                                                 \
  if (x < 0)                                                                   \
    return false;
#define IF_NEG_RETURN_MINUS1(x)                                                \
  if (x < 0)                                                                   \
    return -1;
...
bool local_tpm::send_command(int size, byte_t *command) {
  ...
  if (n < 0)
    printf("%s() error, line %d, send_command Error: %s\n",
           __func__,
           __LINE__,
           strerror(errno));
  return n > 0;
...
int Tpm2_SetCommand(uint16_t tag,
                    uint32_t cmd,
                    byte_t  *buf,
                    int      size_param,
                    byte_t  *params) {
...
bool compute_pcr_digest(TPM_ALG_ID hash,
  ...
  if (hash != TPM_ALG_SHA1 && hash != TPM_ALG_SHA256) {
    printf(
        "%s() error, line %d, compute_pcr_digest: unsupported hash algorithm\n",
        __func__,
        __LINE__);
    return false;
  }
...
bool Tpm2_Startup(local_tpm &tpm) {
  ...
  uint32_t responseSize;
  uint32_t responseCode;
  Tpm2_InterpretResponse(resp_size,
                         resp_buf,
                         &cap,
                         &responseSize,
                         &responseCode);
...
bool Tpm2_GetCapability(local_tpm &tpm,
                        uint32_t   cap,
                        uint32_t   start,
                        int       *out_size,
                        byte_t    *out_buf) {
...
bool Tpm2_ReadClock(local_tpm &tpm,
  ...
  if (responseCode != TPM_RC_SUCCESS)
    return false;
  change_endian64(&((TPMS_TIME_INFO *)(resp_buf + sizeof(TPM_RESPONSE)))->time,
                  current_time);
  change_endian64(
      (&((TPMS_TIME_INFO *)(resp_buf + sizeof(TPM_RESPONSE)))->clockInfo.clock),
      current_clock);
  return true;
}
...
bool Tpm2_ReadPcrs(local_tpm          &tpm,
  ...
  *in = pcrSelect.pcrSelections[0].sizeofSelect;
  ...
  int cmd_size = Tpm2_SetCommand(TPM_ST_NO_SESSIONS,
                                 TPM_CC_PCR_Read,
                                 commandBuf,
                                 in_size,
                                 input_params);
...
bool Tpm2_ReadPcr(local_tpm          &tpm,
                  int                 pcrNum,
                  uint32_t           *updateCounter,
                  TPML_PCR_SELECTION *pcrSelectOut,
                  TPML_DIGEST        *values) {
  ...
  return Tpm2_ReadPcrs(tpm, pcrSelect, updateCounter, pcrSelectOut, values);
}
...
int SetPasswordData(string &password, int size, byte_t *buf) {
  ...
  if (num_auth_bytes > 0) {
    IF_LESS_THAN_RETURN_FALSE(space_left, num_auth_bytes)
...
int CreateSensitiveArea(int     size_in,
                        byte_t *in,
                        int     size_data,
                        byte_t *data,
                        int     size,
  ...
  Update(sizeof(uint16_t), &out, &total_size, &space_left);
  if (size_in > 0) {
    IF_LESS_THAN_RETURN_FALSE(size_in, sizeof(uint16_t));
  ...
  Update(sizeof(uint16_t), &out, &total_size, &space_left);
  if (size_data > 0) {
    IF_LESS_THAN_RETURN_FALSE(size_data, sizeof(uint16_t));
...
int CreateSensitiveArea(string &authString,
  ...
  IF_LESS_THAN_RETURN_FALSE(space_left, authString.size());
  memcpy(out, (byte_t *)authString.data(), authString.size());
  Update(authString.size(), &out, &total_size, &space_left);

  IF_LESS_THAN_RETURN_FALSE(space_left, sizeof(uint16_t));
  uint16_t size_data = (uint16_t)sensitiveData.size();
  change_endian16(&size_data, (uint16_t *)out);
  Update(sizeof(uint16_t), &out, &total_size, &space_left);

  IF_LESS_THAN_RETURN_FALSE(space_left, sensitiveData.size());
  memcpy(out, (byte_t *)sensitiveData.data(), sensitiveData.size());
  Update(sensitiveData.size(), &out, &total_size, &space_left);
...
int CreateSensitiveAreaWithSize(string &authString,
  ...
  uint16_t size_area = sizeof(uint16_t) + authString.size() + sizeof(uint16_t)
                       + sensitiveData.size();
  change_endian16((uint16_t *)&size_area, (uint16_t *)pSize);
  return total_size;
}

bool Tpm2_PCR_Event(local_tpm &tpm,
                    int        pcr_num,
                    uint16_t   size_eventData,
                    byte_t    *eventData) {
  byte_t  commandBuf[2 * MAX_SIZE_PARAMS];
  int     input_size = 0;
  int     space_left = MAX_SIZE_PARAMS;
...
int Marshal_AuthSession_Info(TPMI_DH_OBJECT         &tpm_obj,
  ...
  IF_LESS_THAN_RETURN_FALSE(space_left, initial_nonce.size)
  ...
  IF_LESS_THAN_RETURN_FALSE(space_left, salt.size)
...
int Marshal_Public_Key_Info(TPM2B_PUBLIC &in, int size, byte_t *buf) {
  ...
  // authPolicy
  IF_LESS_THAN_RETURN_FALSE(space_left, sizeof(uint16_t))
  change_endian16((uint16_t *)&in.publicArea.authPolicy.size, (uint16_t *)out);
  Update(sizeof(uint16_t), &out, &total_size, &space_left);
  ...
  // Public Id
  IF_LESS_THAN_RETURN_FALSE(space_left, sizeof(uint16_t))
  change_endian16((uint16_t *)&in.publicArea.unique.sym.size, (uint16_t *)out);
  Update(sizeof(uint16_t), &out, &total_size, &space_left);
...
int Marshal_OutsideInfo(TPM2B_DATA &in, int size, byte_t *buf) {
  ...
  IF_LESS_THAN_RETURN_FALSE(space_left, in.size)
...
int Marshal_Signature_Scheme_Info(TPMT_SIG_SCHEME &sig_scheme,
                                  int              size,
                                  byte_t          *buf) {
  int     total_size = 0;
  int     space_left = size;
...
int Marshal_Keyed_Hash_Info(TPM2B_PUBLIC &keyed_hash, int size, byte_t *buf) {
  ...
  // policy
  IF_LESS_THAN_RETURN_FALSE(space_left, sizeof(uint16_t))
  change_endian16(&keyed_hash.publicArea.authPolicy.size, (uint16_t *)out);
  Update(sizeof(uint16_t), &out, &total_size, &space_left);

  IF_LESS_THAN_RETURN_FALSE(space_left, keyed_hash.publicArea.authPolicy.size)
  memcpy(out,
         keyed_hash.publicArea.authPolicy.buffer,
  ...
  IF_LESS_THAN_RETURN_FALSE(space_left, sizeof(uint16_t))
  change_endian16(
      &keyed_hash.publicArea.parameters.keyedHashDetail.scheme.scheme,
      (uint16_t *)out);
  Update(sizeof(uint16_t), &out, &total_size, &space_left);
...
void FillPublicRsaTemplate(TPM_ALG_ID        enc_alg,
                           TPM_ALG_ID        int_alg,
                           TPMA_OBJECT       flags,
                           string           &policyString,
                           TPM_ALG_ID        sym_alg,
  ...
  pub_key.publicArea.authPolicy.size = policyString.size();
  memcpy(pub_key.publicArea.authPolicy.buffer,
         (byte_t *)policyString.data(),
         policyString.size());
...
void FillSignatureSchemeTemplate(TPM_ALG_ID       enc_alg,
                                 TPM_ALG_ID       int_alg,
...
void FillKeyedHashTemplate(TPM_ALG_ID    enc_alg,
                           TPM_ALG_ID    int_alg,
                           TPMA_OBJECT   flags,
                           uint16_t      size_auth,
                           byte_t       *auth,
...
bool Tpm2_CreatePrimary(local_tpm          &tpm,
  ...
  int     size = responseSize - sizeof(TPM_RESPONSE);
...
bool Tpm2_PolicyGetDigest(local_tpm    &tpm,
                          TPM_HANDLE    handle,
...
bool Tpm2_StartAuthSession(local_tpm              &tpm,
                           TPM_RH                  tpm_obj,
                           TPM_RH                  bind_obj,
                           TPM2B_NONCE            &initial_nonce,
                           TPM2B_ENCRYPTED_SECRET &salt,
  ...
  memset(params, 0, MAX_SIZE_PARAMS);
  int n = Marshal_AuthSession_Info(tpm_obj,
                                   bind_obj,
                                   initial_nonce,
                                   salt,
                                   session_type,
...
bool Tpm2_PolicyPcr(local_tpm          &tpm,
                    TPM_HANDLE          session_handle,
...
bool Tpm2_Save(local_tpm &tpm) {
  printf("%s() error, line %d, Unimplmented\n", __func__, __LINE__);
  return false;
}
...
bool get_public_out(uint16_t size_in, byte_t *input, TPM2B_PUBLIC *outPublic) {
  ...
  input += sizeof(uint16_t);
  size_in -= sizeof(uint16_t);
  memcpy(outPublic->publicArea.authPolicy.buffer,
         input,
  ...
  if (n < 0) {
    printf("%s() error, line %d, Can't get RSA Params\n", __func__, __LINE__);
    return false;
  }
  input += n;
  return true;
...
bool Tpm2_Certify(local_tpm      &tpm,
                  ...
                  string         &auth_signed_key,
                  string         &auth_signing_key,
  ...
  *in = 1;
  ...
  out += sizeof(uint16_t);
  memcpy(sig->signature.rsassa.sig.buffer, out, sig->signature.rsassa.sig.size);
  out += sizeof(sig->signature.rsassa.sig.size);
  return true;
}

bool GetCreateOut(int                  size,
                  byte_t              *in,
                  int                 *size_public,
                  byte_t              *out_public,
                  int                 *size_private,
  ...
  *size_public = 0;
  ...
  current_in += sizeof(uint16_t);
  memcpy(creation_out->creationData.pcrDigest.buffer,
         current_in,
         creation_out->creationData.pcrDigest.size);
...
bool ComputeHmac(int          size_buf,
                 byte_t      *command,
                 TPM2B_NONCE &newNonce,
                 TPM2B_NONCE &oldNonce,
                 uint16_t    *size_hmac,
                 byte_t      *hmac) {
...
bool Tpm2_LoadContext(local_tpm  &tpm,
                      uint16_t    size,
                      byte_t     *saveArea,
                      TPM_HANDLE *handle) {
  byte_t commandBuf[2 * MAX_SIZE_PARAMS];
...
bool Tpm2_SaveContext(local_tpm &tpm,
                      TPM_HANDLE handle,
                      uint16_t  *size,
                      byte_t    *saveArea) {
  byte_t commandBuf[2 * MAX_SIZE_PARAMS];
...
bool Tpm2_WriteNv(local_tpm       &tpm,
                  TPMI_RH_NV_INDEX index,
                  string          &authString,
                  uint16_t         size,
                  byte_t          *data) {
...
bool Tpm2_DictionaryAttackLockReset(local_tpm &tpm) {
  ...
  Tpm2_InterpretResponse(size_resp,
                         ...
                         &responseCode);
#ifdef DEBUG
  print_response("DictionaryAttackLockReset",
                 cap,
                 responseSize,
                 responseCode,
                 resp_buf);
...
bool Tpm2_Flushall(local_tpm &tpm, uint32_t start) {
  ...
  if (!Tpm2_GetCapability(tpm, TPM_CAP_HANDLES, start, &size, buf)) {
    printf("%s() error, line %d, Flushall can't get capabilities\n",
           __func__,
           __LINE__);
    return false;
  }
  ...
  current_in += sizeof(uint32_t);
  if (cap != TPM_CAP_HANDLES) {
    printf("%s() error, line %d, Flushall: didn't get handles\n",
           __func__,
           __LINE__);
    return false;
  }
...
bool CalculateNvName(ProtectedSessionAuthInfo &in,
  ...
  if (in.hash_alg_ != TPM_ALG_SHA1 && in.hash_alg_ != TPM_ALG_SHA256) {
    printf("%s() error, line %d, CalculateNvName: unsupported hash algorithm\n",
           __func__,
           __LINE__);
    return false;
  }
...
bool CalculateSessionHmac(ProtectedSessionAuthInfo &in,
                          bool                      dir,
                          uint32_t                  cmd,
                          int                       numNames,
                          TPM2B_NAME               *names,
  ...
  if (in.hash_alg_ != TPM_ALG_SHA1 && in.hash_alg_ != TPM_ALG_SHA256) {
    printf("%s() error, line %d, CalculateSessionHmac: unsupported hash "
           "algorithm\n",
           __func__,
           __LINE__);
    return false;
  ...
  memcpy(&hmac_key[in.sessionKeySize_],
         in.targetAuthValue_.buffer,
...
bool CalculateSessionKey(ProtectedSessionAuthInfo &in, TPM2B_DIGEST &rawSalt) {
  ...
  printf("\n");
  printf("    label  : ");
  print_bytes(label.size(), (byte_t *)label.data());
  printf("\n");
  printf("    U      : ");
...
int CalculateandSetProtectedAuthSize(ProtectedSessionAuthInfo &authInfo,
  ...
  //    zero TPMA_SESSION sessionAttributes the flags associated with the
  //    session TPM2B_AUTH hmac the session HMAC digest value
  return sizeof(uint32_t) + sizeof(TPMI_SH_AUTH_SESSION) + sizeof(uint16_t)
         + authInfo.newNonce_.size + sizeof(byte_t) + sizeof(uint16_t)
         + SizeHash(authInfo.hash_alg_);
}
...
int CalculateandSetProtectedAuth(ProtectedSessionAuthInfo &authInfo,
  ...
  printf("\n");
  printf("oldNonce: ");
  print_bytes(authInfo.oldNonce_.size, authInfo.oldNonce_.buffer);
  printf("\n");
#endif

  int    sizeHmac = SizeHash(authInfo.hash_alg_);
  byte_t hmac[128];
  if (!CalculateSessionHmac(authInfo,
                            false,
                            cmd,
                            numNames,
                            names,
                            ...
                            &sizeHmac,
                            hmac)) {
    printf("%s() error, line %d, CalculateSessionHmac failed\n",
           __func__,
           __LINE__);
    return false;
  }
...
bool GetandVerifyProtectedAuth(ProtectedSessionAuthInfo &authInfo,
                               uint32_t                  cmd,
                               int                       numNames,
                               TPM2B_NAME               *names,
                               int                       size_in,
  ...
  print_bytes(size_params, params);
  printf("\n");
#endif
  ...
  printf("\n");
  printf("submitted: ");
  print_bytes(submitted_hmac.size, submitted_hmac.buffer);
  printf("\n");
#endif
  ...
  if (!CalculateSessionHmac(authInfo,
                            ...
                            &sizeHmac,
                            hmac)) {
    printf("%s() error, line %d, Can't calculate response hmac\n",
           __func__,
           __LINE__);
    return false;
  }
  if (submitted_hmac.size != sizeHmac
    ...
    printf("%s() error, line %d, Response hmac failure\n", __func__, __LINE__);
    return false;
  }
  return true;
}
...
bool Tpm2_DefineProtectedSpace(local_tpm                &tpm,
  ...
  // Set auth
  int n = CalculateandSetProtectedAuthSize(authInfo,
                                           TPM_CC_NV_DefineSpace,
                                           0,
                                           nullptr,
                                           in,
                                           space_left);
  if (n < 0) {
    printf("%s() error, line %d, CalculateandSetProtectedAuthSize failed\n",
           __func__,
           __LINE__);
    return false;
  }
  ...
  // Set auth
  n = CalculateandSetProtectedAuth(authInfo,
                                   TPM_CC_NV_DefineSpace,
                                   1,
                                   &owner_name,
                                   size_append,
  ...
  if (!GetandVerifyProtectedAuth(authInfo,
                                 TPM_CC_NV_DefineSpace,
                                 0,
                                 &no_name,
                                 responseSize - sizeof(TPM_RESPONSE),
                                 resp_buf + sizeof(TPM_RESPONSE),
                                 size_resp_params,
                                 resp_params)) {
    printf("%s() error, line %d, GetandVerifyProtectedAuth failed\n",
           __func__,
           __LINE__);
    return false;
  }
...
bool Tpm2_IncrementProtectedNv(local_tpm                &tpm,
                               TPMI_RH_NV_INDEX          index,
                               ProtectedSessionAuthInfo &authInfo) {
  byte_t  commandBuf[2 * MAX_SIZE_PARAMS];
  int     size_resp = MAX_SIZE_PARAMS;
  ...
  if (!CalculateNvName(authInfo,
                       index,
                       authInfo.hash_alg_,
                       authInfo.protectedAttributes_,
                       authInfo.protectedSize_,
                       true,
                       nv_name[0].name)) {
  }
  ...
  int n = CalculateandSetProtectedAuthSize(authInfo,
                                           ...
                                           space_left);
  if (n < 0) {
    printf("%s() error, line %d, CalculateandSetProtectedAuth failed\n",
           __func__,
           __LINE__);
    return false;
  }
  ...
  Tpm2_InterpretResponse(size_resp,
                         ...
                         &responseCode);
#ifdef DEBUG
  print_response("IncrementProtectedNv",
                 cap,
                 responseSize,
                 responseCode,
                 resp_buf);
