...
class local_tpm {
 public:
  ...
  bool open_tpm(const char *device);
  void close_tpm();
  bool send_command(int size, byte_t *command);
  bool get_response(int *size, byte_t *response);
};

// Helpers
int  Tpm2_SetCommand(TPM_ST   tag,
                     uint32_t cmd,
                     byte_t  *buf,
                     int      size_param,
                     byte_t  *params);
void Tpm2_IntepretResponse(int       out_size,
                           byte_t   *out_buf,
                           int16_t  *cap,
